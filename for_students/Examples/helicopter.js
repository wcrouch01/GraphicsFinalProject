...
export class Helipad extends GrObject {
    ...
    constructor(x,y,z) {
        ...
        if (!helipadMaterial) {
            helipadMaterial = new T.MeshLambertMaterial({color:"#FFFF00", side:T.DoubleSide});
        }
...
export class Helicopter extends GrObject {
    ...
    constructor(params={}) {
        ...
        let material = new T.MeshStandardMaterial({color:"#88CC22",side : T.DoubleSide});
        ...
        for(let i=0; i<helicoords.length; i+=3) {
            helicopterBodyGeom.vertices.push(new T.Vector3( helicoords[i],  helicoords[i+1], helicoords[i+2] ));
        }
        for (let i=0; i<helifaces.length; i+=3) {
            helicopterBodyGeom.faces.push(new T.Face3( helifaces[i], helifaces[i+1], helifaces[i+2] ));
        }
        ...
        for(let i=0; i<rotorcoords.length; i+=3) {
            this.rotorGeom.vertices.push(new T.Vector3( rotorcoords[i],  rotorcoords[i+1], rotorcoords[i+2] ));
        }
        for (let i=0; i<rotorfaces.length; i+=3) {
            this.rotorGeom.faces.push(new T.Face3( rotorfaces[i], rotorfaces[i+1], rotorfaces[i+2] ));
        }
